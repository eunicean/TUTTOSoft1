...
const Sidebar = ({ isOpen, closeSidebar }) => {
  const [typeuser, setTypeuser] = useState(null); // Estado para almacenar el tipo de usuario
  ...
  useEffect(() => {
    async function fetchUserProfile() {
      ...
      try {
        ...
        if (data.success) {
          setTypeuser(data.user.typeuser); // Almacena el tipo de usuario en el estado
        } else {
    ...
    fetchUserProfile(); // Llama a la función para obtener el perfil al montar el componente
  }, [navigate]);


  const handleLogout = () => {
    localStorage.removeItem('token'); // Eliminar el token de autenticación
    localStorage.removeItem('typeuser'); // Eliminar el tipo de usuario
    ...
    <div className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <ul className="sidebar-items">
        ...
        {typeuser === '3' ? (
          <>
            <li>
              <Link to="/adminsearch" onClick={closeSidebar} className="sidebar-link">
            ...
            <li>
              <Link to="/admintutor" onClick={closeSidebar} className="sidebar-link">
          ...
          // Menú para otros usuarios (Estudiante o Tutor)
          <>
            <li>
