...
const Chat = () => {
  ...
  // Función para obtener los mensajes de un chat específico y agregar el último mensaje si se selecciona desde el historial
  const fetchMessages = async (chatId, lastMessage = null) => {
    try {
      ...
      if (response.data.success) {
        ...
        if (lastMessage) {
          // Agregar el último mensaje al historial de mensajes
          fetchedMessages = [...fetchedMessages, { senderId: 1, content: lastMessage }];
        }
        setMessages(fetchedMessages);  // Actualiza los mensajes en el estado
        setSelectedChat(chatId);  // Marca el chat como seleccionado
      } else {
        setMessages([]); // Si no hay mensajes, vacía el estado
      }
    } catch (error) {
      ...
      try {
        ...
        if (response.data.success) {
          setMessages((prevMessages) => [
            ...prevMessages,
            { content: inputMessage, senderUsername: 'Tú' },
          ]); // Añade el mensaje al estado
          setInputMessage(''); // Limpiar el campo de entrada
          setStatusMessage('Mensaje enviado');
          setTimeout(() => setStatusMessage(''), 2000);
          await fetchMessages(selectedChat); // Actualiza la lista de mensajes después de enviar uno nuevo
        }
      } catch (error) {
        ...
        <ul>
          {chats.length > 0 ? (
            chats.map((chat) => (
              <li
                key={chat.chatId}
                onClick={() => fetchMessages(chat.chatId, chat.lastMessage)} // Pasar el último mensaje del historial
            ...
            <div className="chat-messages">
              {messages.length > 0 ? (
                messages.map((msg, index) => (
                  <div key={index} className={`chat-message ${msg.senderUsername === 'Tú' ? 'user-message' : 'other-message'}`}>
                    {/* ID del remitente en negrita y fuera de la burbuja de mensaje */}
                    <div className="sender-info" style={{ color: '#8B4513', fontWeight: 'bold' }}>
                      {msg.senderUsername === 'Tú' ? 'Tú' : `ID: ${msg.senderId}`}
                    </div>
